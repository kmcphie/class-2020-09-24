...
```{r}
sep_join = left_join(sep_old, sep_new, by = c("id", "title", "name", "department"), suffix = c("_old", "_new"))
View(sep_join)
```
...
```{r}
sep_join_inc <- sep_join %>%
  select(id, name, u_grad_old, u_grad_new) %>%
...
```{r}
sep_join_ten <- rbind(sep_join_inc, sep_join_dec) %>%
View(sep_join_ten)

sep_join_ten %>%
  ggplot(aes(x = fct_reorder(name, enrollment_diff), y = enrollment_diff)) +
    geom_col(fill = "blue") +
    coord_flip() +
    labs(
